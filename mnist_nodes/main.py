...
class Network():
    def __init__(self):
        ...
        for layer_index in range(len(self.amount)):
            for node_index in range(self.amount[layer_index]):
                self.network[layer_index].append(deepcopy(Node(self.number, layer_index)))
                self.number += 1
        
        for layer_index in range(len(self.amount)-1):
            for node in self.network[layer_index]:
                for following_node in self.network[layer_index+1]:
                    node.connections.append(deepcopy(Connection(following_node.identity, layer_index+1)))
    ...
    def forward(self, image):
        ...
        values = []
        for node in self.network[len(self.network)-1]: values.append(np.tanh(node.value))
        return values
...
for epoch in range(epochs):
    ...
    for i in tqdm(range(len(images))):
        ...
        network.backward(value - ground_truth)
...
plt.scatter(episodes, errors, color= "black", marker= "x")
plt.show()
